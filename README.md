...
You can also retrieve nested fields of the generated JSon. Just follow Apache commons Bean Utils syntax, for example: 
...
Please note that the parameter ${WireMockCsv} represents the entire JSon generated by the request.
It's possible to retrieve partial results from the result in order to personnalize more precisely the structure. Apache commons Bean Utils syntax still applies.

This is particularly interesting for the request specific override.

For example, if the JSon result is:
...
See the example "extraction2" to see it in action.
...
* "considerAsJson": A list of column names of the query results which will be considered as JSon Strings. Meaning that they will be parsed into Objects and will appear as part of the JSon result.
...
Wiremock CSV allows to chain several extensions between them. Two modes:

* Use the data rendered by a previous extension.
* Provide data for the future rendering of a following extension.

These two modes are not exclusive: it's possible to cumulate the behaviors.
...
The data rendered by WireMockCsv will be included in the data rendered by the previous extension.

Only one parameter must be activated: `useResponseBodyAsStructure`. This parameter allows to specify to WireMockCsv that the structure template is provided by a previously executed extension.
The temporary body of the response provided as a parameter to the extension is not overriden but used as a template.

This body must include the variable `${WireMockCsv}` so that it's replaced by the content generated by the WireMockCsv extension.
...
For a following extension to use the content generated by WireMockCsv, this extension should be able to read / modify the Response body generated by WireMockCsv.

No particular action is to be taken in WireMockCsv.
...
* Extract data, by specifying only sub-queries, and personilizing the template structure
