...
* "query": A SQL query, potentially parameterized with HTTP request parameters or "mother" query results (for sub-queries):
    ...
    * Following syntax for parameters allow to retrieve values from http request body : @{$..jsonObject} or @[$..jsonObject] to escape quotes in SQL strings.
...
* Extract value from http request body :
...
* Fix previous evolution added in 1.1.2 :
  - Handling arrays
  - Modifying syntax (now using @{...} for JSon parameters)
  - Handling JSon parameters in custom parameters source
